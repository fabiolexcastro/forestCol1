...
grps <- tibble(value = 1:5, categoria = c('Bosque estable', 'No bosque', 'Bosque nuevo', 'Sin información', 'No bosque estable'), nueva_cat = c('Bosque', 'No bosque', 'Bosque', 'Sin información', 'No bosque'))
...
clipfunc <- function(year){
  ...
  purrr::map(.x = 1:nct, .f = function(i){
    ...
    # Group
    fq <- inner_join(fq, grps, by = c('Value' = 'value', 'Categoria' = 'categoria'))
    fq <- fq %>% 
      group_by(Nombre, nueva_cat, year) %>% 
      dplyr::summarise(nPixeles = sum(nPixeles), nPixelesBuffer = sum(nPixelesBuffer), area_project_ha = mean(area_project_ha), aera_ha = sum(area_ha), area_buffer_ha = sum(area_buffer_ha)) %>% 
      ungroup()
    ...
    dir_create(dr)
    do <- gsub('.tif', '.csv', do)
    write.csv(fq, glue('{dr}/grps_{do}'), row.names = F)
